	...
	extends Module_Members_Bulk 
{
	...
	function importFile($file)
	{
		...
		if ($is_upload)
		{
			$file_content = file('uplimg/' . $file['name']);
			...
			foreach ($file_content as $id => $row)
			{
				...
				if ($id > 0)
				{
					...
					foreach ($this->fields_details as $details)
					{
						...
						if (!empty($details['formchecker']))
						{
							foreach ($details['formchecker'] as $form_checker)
							{
								...
								if (!$fv_result)
								{
									echo 'RESULT: ' . $id . ' | ' . $function_name . ' | ' . $field . ' | ' . print_r($allowed_values) . '<br>';
								}
					...
					if ($import_errors)
					{
						$this->setException('<span class="redfont"><b>Error</b>:</span> Errors reported on row <b>#' . ($id+1) . '</b>, Auction Name: <b>' . $imported_row[0] . '</b>. The row cannot imported.');
					}
					else 
					{
