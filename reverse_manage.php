...
##-------------------------------------------------------------##
## Copyright ©YPBAY!2009 PHP Pro Software LTD. All rights reserved.	##
##-------------------------------------------------------------##
#################################################################
...
else
{
	...
	{ ## <<BEGIN>> media upload sequence
		...
		if ($setup_fee['amount'])
		{
			$setup_fee_expl_message = GMSG_AUCTION_SETUP_FEE . ': (<b>' . $fees->display_amount($setup_fee['amount'], $setts['currency'], true) . '</b>)';
			$template->set('setup_fee_expl_message', $setup_fee_expl_message);
		}

		$addlcat_fee = $reverse_fee->display_fee('second_cat_fee', $user_details, $item_details['category_id'], 'auction');
	
		if ($addlcat_fee['amount'])
		{
			$addlcat_fee_expl_message = '(<b>+' . $fees->display_amount($addlcat_fee['amount'], $setts['currency'], true) . '</b>)';
			$template->set('addlcat_fee_expl_message', $addlcat_fee_expl_message);
		}
		...
		if ($dd_fee['amount'])
		{
			$dd_fee_expl_message = '(<b>+' . $fees->display_amount($dd_fee['amount'], $setts['currency'], true) . '</b>)';
			$template->set('dd_fee_expl_message', $dd_fee_expl_message);
		}		
