...
function get_item_types()
{
   ...
   $output[] = array(
      ...
      'query' => 'a.buyout_price!=a.start_price'
   );
...
function set_srcbox_categories($src_details)
{
	...
	$user_query = (IS_SHOP) ? " AND (user_id=0 OR user_id='" . intval($src_details['user_id']) . "')" : " AND user_id=0";
	...
	$sql_select_categories = $db->query("SELECT category_id FROM " . DB_PREFIX . "categories 
		WHERE parent_id='" . intval($src_details['parent_id']) . "' AND enable_auctions=1 
		" . $user_query . "		
		" . $categories_query . "		
