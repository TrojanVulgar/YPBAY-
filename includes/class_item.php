...
class item extends custom_field
{
	...
	function insert($variables_array, $owner_id, $page_handle = 'auction', $draft = false, $admin_edit = false, $edit_table = 'auctions')
	{
		...
		$sql_insert_item = $this->query("UPDATE " . DB_PREFIX . $edit_table . " SET
			...
			fb_current_bid='" . doubleval($item_details['fb_current_bid']) . "', 
			seller_ip='" . $this->rem_special_chars($_SERVER['REMOTE_ADDR']) . "' 
	...
	function durations_drop_down($box_name = 'duration', $selected = null, $form_refresh = null, $user_id = null, $category_id = null, $drop_down = true, $add_default = false, $list_in = null)
	{
		...
		else 
		{
			...
			while ($duration_details = $this->fetch_array($sql_select_durations))
			{
				...
				$duration_description = $duration_details['description'] . (($this->fees->fee['duration'][$duration_details['id']] > 0 && ($user_id || !$drop_down)) ? ' - (+' . $this->fees->display_amount($this->fees->fee['duration'][$duration_details['id']]) . ')' : '');
				
				if ($drop_down)
				{
					$display_output .= '<option value="' . $duration_details['days'] . '" ' . (($selected_value) ? 'selected' : '') . '>' . $duration_description . '</option> ';
				}
				else if ($this->fees->fee['duration'][$duration_details['id']] > 0)
				{
   ...
   function set_upload_id($variables_array)
   {
	...
	function upload_file($item_id, $media_type, $file_details, $file_prefix, $create_row = true, $bulk_lister = false)
	{
		...
		if ($media_type == 3 && !$bulk_lister)
		{
			...
			$file_name = ereg_replace('.' . $file_extension, '', $file_details['name']);
		}
	 ...
	 */
	function item_media_thumbnails($variables_array, $file_type, $enable_links = true, $reverse_auction = false, $direction = 'v')
	{
		(string) $display_output = null;
		...
		if ($nb_uploads>0)
		{
			...
			foreach ($variables_array[$media_name] as $value)
			{
				...
				if (!empty($value))
				{
					if ($file_type == 1)
					{
						$display_output .= (($direction == 'v') ? '<tr>' : '') . '<td> ';
						$display_output .= ($enable_links) ? '<a href="' . $value . '" rel="lightbox[item_images]" onmouseover="doPic(\'thumbnail.php?pic=' . $value . '&w=500&sq=Y&b=Y\');"> ' : '';
						$display_output .= '<img src="thumbnail.php?pic=' . $value . '&w=100&sq=Y&b=Y" border="0">';
						$display_output .= ($enable_links) ? '</a>' : '';
						$display_output .= '</td>' . (($direction == 'v') ? '</tr>' : '');
					}
					else if ($file_type == 2)
					{
						$display_output .= (($direction == 'v') ? '<tr>' : '') . '<td> ';
					...
					else if ($file_type == 3)
					{
						if ($reverse_auction)
						{
							...
							$display_output .= (($direction == 'v') ? '<tr>' : '') . '<td class="contentfont"> ';
						...
						{
							$display_output .= (($direction == 'v') ? '<tr>' : '') . '<td align="center"> ';
		...
		if ($nb_images > 0)
		{
			foreach ($variables_array['ad_image'] as $value)
			{
				if (!empty($value))
				{
					...
					$display_output .= '<span class="contentfont"><a href="' . $value . '" rel="lightbox[item_images]">' . $link_text . '</a></span>';
				}
			}
			*/
	...
	function video_box($file_name)
	{
		...
		if (intval($file_name) > 0 || eregi('<object', $this->add_special_chars($file_name)))
		{
			...
			$embedded_code = strip_tags($embedded_code, '<param><object><embed>');
	...
	function under_time($item_details)
	{
		...
		return ($time_left>CURRENT_TIME || $item_details['auction_type'] == 'first_bidder' || $item_details['list_in'] == 'store') ? false : true;
	}
	...
	function can_close_manually($item_details, $user_id)
	{
		...
		return (($this->setts['end_auction_early'] && !$item_details['closed']) || 
			(($time_left>CURRENT_TIME || $item_details['auction_type'] == 'first_bidder') && 
	...
	function relist($auction_id, $user_id, $duration = 0, $charge_fees=true, $auto_relist = false)
	{
		...
		if ($can_list)
		{
			...
			$relist_field[] = "end_time = '" . (($relist_details['auction_type'] == 'first_bidder' || $relist_details['list_in'] == 'store') ? 0 : $auction_end_time) . "'";
...
//		}
	...
	{		
		if (eregi($this->setts['dd_folder'], $name))
		{
