...
function calculate_postage($winner_ids, $seller_id, $auction_id = null, $buyer_id = null, $buyer_country = null, $buyer_state = null, $sc_quantity = 0, $sc_carrier = null, $buyer_zip_code = null)
{
	...
	if ($auction_id)
	{
		$sql_select_winners = $db->query("SELECT a.postage_amount AS item_postage,
			a.insurance_amount AS item_insurance, a.item_weight, a.currency, a.country, a.state, a.zip_code  
...
function category_box ($category_id, $parent_id, $target_box_id, $prefix, $reverse_categories, $select_id, $user_id = 0, $listing_type = 'auction')
{
	...
	$output = '<div class="category_box"><select size="8" style="width: 225px;" onchange="select_category(this.value, \'' . $target_box_id . '\', \'' . $prefix . '\', ' . $reverse_categories . ', false, \'' . $listing_type . '\');">';
	...
	$sql_select_cats = $db->query("SELECT category_id, is_subcat FROM " . DB_PREFIX . $categories_table . "
		...
		" . (($reverse_categories) ? '' : " AND user_id IN (0, " . $user_id . ") ") . "
		" . $select_which . " ORDER BY order_id ASC, name ASC");
...
function generate_category_boxes($category_id, $target_box_id, $prefix = 'main_', $reverse_categories = 0, $click_select = false, $user_id = 0, $listing_type = 'auction')
{
	...
	if($category_id > 0)
	{
		...
		while ($root_id > 0)
		{
			$row_category = $db->get_sql_row("SELECT c.category_id, c.name, c.parent_id, 
				...
				" . (($reverse_categories) ? '' : " AND c.user_id IN (0, " . $user_id . ") ") . " LIMIT 0,1");
		...
		{
			$output .= category_box($category['category_id'], $category['parent_id'], $target_box_id, $prefix, $reverse_categories, $select_id, $user_id, $listing_type);			
		}
		...
		{
			$output .= category_box(0, $last_category['category_id'], $target_box_id, $prefix, $reverse_categories, $select_id, $user_id, $listing_type);
		}		
	...
	{
		$output = category_box(0, 0, $target_box_id, $prefix, $reverse_categories, $select_id, $user_id, $listing_type);
	}
