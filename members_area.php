...
else
{
	...
	if ($page == 'bidding' || $page == 'selling' || $page == 'summary') /* allow bidders to create product invoices as well */
	{
		if ($page == 'selling' || $page == 'summary')
		{
			...
			(ao.offer_id IS NOT NULL OR s.swap_id IS NOT NULL) AND ";
		}
		...
		$nb_items_bids = $db->get_sql_number("SELECT count(*) FROM " . DB_PREFIX . "auctions a
			...
			WHERE 
			" . $user_query . " 
			a.closed=0 AND a.deleted=0 AND a.creation_in_progress=0 AND a.is_draft=0 AND a.end_time>0 AND 
		...
		if ($section == 'closed')
		{
			...
			if ($_REQUEST['show'] == 'sold')
			{
				...
				$nb_closed_items = $db->get_sql_number("SELECT a.auction_id FROM " . DB_PREFIX . "auctions a 
					...
					a.deleted=0 AND a.end_time<='" . CURRENT_TIME . "' AND a.creation_in_progress=0 AND a.is_draft=0" . $search_filter . " GROUP BY a.auction_id");
			}
	...
	if ($page == 'bulk') /* BEGIN -> BULK PAGE(S) */
	{
		if ($section == 'details')
		{
			...
			if (isset($_POST['form_bulk_list_proceed']) || isset($_POST['form_bulk_delete_all']))
			{
				...
				$msg_changes_saved = '<p align="center">' . MSG_PENDING_BULK_LISTINGS_DELETED . '</p>';
				$template->set('msg_changes_saved', $msg_changes_saved);				
			}
			...
			if (isset($_POST['form_bulk_upload']))
			{
				...
				{
					$bulk_listing_process_output .= MSG_IMPORT_SUCCESSFUL;
				}
			}
			...
			if ($pending_bulk_items)
			{
				...
				while ($item_details = $db->fetch_array($sql_select_items))
				{
					...
					$fee_output = $setup_fee->setup($user_details, $item_details);
